...
As Eq. 1 shows, the input of layer l, denoted by \\(\bar{a}^{[l-1]}\\), is the output generated by layer l-1, so we can write the equation for \\(\bar{a}^{[l]}\\) as a composition of all previous layers, 1<=l, as expressed by Eq. 2.
...
The Gradient Descet equations are presented by Eq. 4, Where the superscript [l], l=[1,L], denotes the layer, and \\(\alpha\\) is the learning rate.
...
As shown by Eq. 4, to solve the recursive equations, Cost function's derivatives with respect to all layers' coeffcients are needed. 

How can \\(\alpha\frac{\partial C}{\partial w^{[l]}}\\) and \\(\alpha\frac{\partial C}{\partial b^{[l]}}\\) be calculated for all  1<=l<=L? Let's continue!

## Backwards Propogation Algorithm


Since \\(C(y,\hat{y})\\) is a function of \\(\hat{y}\\), and since \\(\hat{y}=a^{[L]}\\), then equivalently the cost is \\(C(y,a^{[L]})\\), and we can plug in Eq.2 with l=L to have:


\\(C(y, g^{[L]}(w^{[L]}g^{[L-1]}(w^{[L-1]}g^{[L-2]}(w^{[L-2]}g^{[L-3]}(......g^{[1]}(w^{[1]}a^{[0]}))))\\)
...
Cwe can use the derivative chain rule and write the following derivative equations:
